...
with (import <nixpkgs> {});
let
  fileSplitLines = file: # file: Path -> fileSplitContents: List[Str]
    lib.splitString "\n" (lib.fileContents file);

  parseGameData = fileSplitContents: # fileSplitContents: List[Str] -> attrset
    let 
      keyValSplitList = map (line: lib.splitString ": " line) fileSplitContents;
      parseGameId = gameIdStr: 
  ...
  validateSet = set:
    lib.foldl lib.and true (lib.mapAttrsToList (color: count: 
      ((lib.getAttrFromPath [color] bagState) >= count))
    set);

  validateSingleGame = game:
    builtins.all validateSet game.sets;

  sumValidGames = games:
    lib.foldl builtins.add 0
      (map (game: game.gameId) (builtins.filter validateSingleGame games));

  computeCubePower = game:
    lib.foldl builtins.mul 1 (lib.attrValues (lib.foldAttrs lib.max 0 game.sets));

  sumCubePower = games:
    lib.foldl builtins.add 0 (builtins.map computeCubePower games);

in {
  outputPart1 = sumValidGames (parseGameData (fileSplitLines ./input));
  outputPart2 = sumCubePower (parseGameData (fileSplitLines ./input));
}
